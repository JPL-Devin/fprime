...
Fw::SerializeStatus AMPCSSequence ::deserializeCmdLength(Record::CmdLength::t& cmdLength) {
    ...
    if (status == Fw::FW_SERIALIZE_OK and (sizeof(FwOpcodeType) - sizeof(U16)) + cmdLength > FW_COM_BUFFER_MAX_SIZE) {
        // Zero-extended opcode plus command bytes are too big for com buffer
        status = Fw::FW_DESERIALIZE_SIZE_MISMATCH;
    }
    return status;
}
