...
## 3. Design
`Svc::OsTime` has two user-facing ports, one `timeGetPort` for the F Prime Time interface and a `setEpoch` port which is described later. It also has a `SetCurrentTime` command and a `SetCurrentTimeError` event.
