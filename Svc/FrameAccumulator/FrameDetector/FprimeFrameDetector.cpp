...
FrameDetector::Status FprimeFrameDetector::detect(const Types::CircularBuffer& data, FwSizeType& size_out) const {
    ...
    // In its current implementation, CircularBuffer is not a SerialBufferBase, which prevents us from deserializing
    ...
    // A better implementation would be to have CircularBuffer implement a shared interface with SerialBufferBase,
