...
| SVC-COMCCSDSSDLS-005 | The default SA map shall contain exactly one entry, mapping a configurable base SA index (`SdlsCfg.SaRouterBaseSa`) to port 0 (the default decryptor).   | Inspection |
...
The `SdlsSaRouter` default configuration contains a single map entry: `{ securityAssociationIndex = SdlsCfg.SaRouterBaseSa, portIndex = 0 }`, with `SdlsCfg.SaRouterBaseSa = 0`. The base SA is configurable by overriding the `SdlsSaRouter` configuration module.
