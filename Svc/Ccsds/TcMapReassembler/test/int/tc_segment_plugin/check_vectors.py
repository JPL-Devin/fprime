...
KAT_CIPHERTEXT = bytes.fromhex("6941202B57CD01F3EF20B5F495AA8FF6A38F6C5D4C3CCC2A824F89F2")
...
def check_kat_vectors() -> None:
    ...
    check("4.6.1 AAD(19)", aad19, _h("00 00 14 00 00 12 34 00 00 00 00 00 00 00 00 00 00 00 00"))
    ...
    for flags, (sh_hex, mac_hex) in expected.items():
        ...
        check(f"4.6.2 AAD SH={sh_hex}", aad, _h("00 00 14 00 00") + bytes([sh]) + _h("12 34") + bytes(12))
...
def check_clear_frames() -> None:
    ...
    assert [s.flags for s in segments] == [tf.FLAG_FIRST, tf.FLAG_CONTINUING, tf.FLAG_LAST]
    frames = [tf.frame_segment(SCID, VCID, fsn, seg) for fsn, seg in enumerate(segments)]
    check("4.6.3a frame 1", frames[0], _h("20 44 04 13 00 40 18 0F C0 07 00 1D 01 02 03 04 05 06 78 66"))
    check("4.6.3a frame 2", frames[1], _h("20 44 04 13 01 00 07 08 09 0A 0B 0C 0D 0E 0F 10 11 12 2D 24"))
    check("4.6.3a frame 3", frames[2], _h("20 44 04 13 02 80 13 14 15 16 17 18 19 1A 1B 1C 1D 1E 77 40"))
    unsegmented = tf.segment_packet(_h("18 0F C0 08 00 05 AA BB CC DD EE FF"), MAP, max_portion=tf.MAX_PORTION_CLEAR)
...
def check_sdls_frames() -> None:
    ...
    check("4.6.3b S1 AAD", tf.sdls_aad(VCID, 1, segments[0].sh), _h("00 00 04 00 00 40 00 01") + bytes(12))
    s1 = tf.frame_segment(SCID, VCID, 0x0A, segments[0], sdls_key=KEY, spi=1, iv_counter=0x0A)
    s2 = tf.frame_segment(SCID, VCID, 0x0B, segments[1], sdls_key=KEY, spi=1, iv_counter=0x0B)
    check(
        "4.6.3b S1",
        s1,
        _h(
    ...
    s2_bad = tf.frame_segment(SCID, VCID, 0x0B, segments[1], sdls_key=KEY, spi=1, iv_counter=0x0B, corrupt_mac=True)
    check(
        "4.6.5 S2'",
        s2_bad,
        _h(
    ...
    unsegmented = tf.segment_packet(_h("18 0F C0 08 00 05 AA BB CC DD EE FF"), MAP, max_portion=tf.MAX_PORTION_SDLS)
    check(
        "4.6.3c VC0 frame",
        tf.frame_segment(SCID, 0, 0x0C, unsegmented[0], sdls_key=KEY, spi=1, iv_counter=0x0C),
        _h(
    ...
    frame = tf.frame_segment(SCID, 5, 0, first, sdls_key=KEY, spi=0x1234, iv_counter=int.from_bytes(KAT_IV, "big"))
    ...
    check("4.6.2 FIRST in frame: MAC", frame[48:64], _h("52 37 0D DF 09 C8 59 AA 82 CB 2A 99 FE 9B D6 08"))


def check_control_frame() -> None:
    # 4.6.4: Type-BC Unlock frame; octet 5 is the FDU, never a Segment Header
    check("4.6.4 Type-BC", tf.control_frame(SCID, VCID, 0), _h("30 44 04 07 00 00 F6 93"))
...
def check_segmentation_bounds() -> None:
    ...
    for size, max_portion in ((1016, tf.MAX_PORTION_CLEAR), (986, tf.MAX_PORTION_SDLS)):
        ...
        assert [s.flags for s in two] == [tf.FLAG_FIRST, tf.FLAG_LAST] and len(two[1].portion) == 1
    three_k = tf.segment_packet(bytes(3000), MAP, tf.MAX_PORTION_SDLS)
    assert [s.flags for s in three_k] == [tf.FLAG_FIRST, tf.FLAG_CONTINUING, tf.FLAG_CONTINUING, tf.FLAG_LAST]
    largest = tf.frame_segment(SCID, VCID, 0, tf.Segment(tf.FLAG_FIRST, MAP, bytes(986)), sdls_key=KEY, spi=1)
