...
The First Header Pointer in the context is forwarded downstream unchanged: it remains relative to the start of the (plaintext) data zone. Per CCSDS 132.0-B-3 the Security Header is a distinct transfer-frame field preceding the Transfer Frame Data Field, so the FHP — measured from the first octet of the Data Field — is unaffected by the prepended SA index. The ground deframer strips the security header and decrypts before applying the FHP to the recovered zone.
