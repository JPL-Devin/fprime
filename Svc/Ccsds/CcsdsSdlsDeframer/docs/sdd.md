...
4. Check the returned status; on failure raise an event, notify `errorNotify`, and return the buffer.
5. Pass the decrypted data downstream for further processing.
...
| SVC-CCSDS-SDLS-DEFRAMER-005 | Upon a non-SUCCESS status from the decryption helper, the CcsdsSdlsDeframer shall emit the `DecryptionFailed` WARNING_HI event, notify `errorNotify` with `SDLS_DECRYPTION_FAILURE`, and return the frame buffer via `dataReturnOut`. | Decryption failures must be visible to the system and the buffer must not leak. | Unit test |
| SVC-CCSDS-SDLS-DEFRAMER-006 | The CcsdsSdlsDeframer shall pass decrypted data received on `decryptIn` downstream via `dataOut`. | Continues the deframing pipeline with plaintext data. | Unit test |
...
| sync input | decryptIn | Svc.ComDataWithContext | Receives decrypted data from the helper. |
