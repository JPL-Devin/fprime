...
The SA-to-port mapping is a compile-time FPP array of {`U16` SA, `FwIndexType` port index} pairs defined in configuration (`SdlsCfg.SaMap`). This permits more SAs than ports, sparse SAs, or project-defined non-linear SA ranges that all map down to a linear, compact array of outputs.

## Functionality

- Receives an SA index and iv/data buffer on the guarded `decryptIn` port.
- Looks up the SA index in the compile-time `SdlsCfg.SaMap`; if found, forwards the request out the mapped `saDecryptOut` port and returns the downstream decryptor's `SdlsStatus` to the caller.
...
- Passes decrypted data emitted by downstream decryptors upstream via `decryptOut`, and routes ownership returns received on `decryptReturnIn` back to the originating decryptor via `saDecryptReturnOut`.
...
| output        | decryptOut         | Fw.BufferSend                   | Sends decrypted data (possibly newly allocated) upstream. |
| sync input    | decryptReturnIn    | Fw.BufferSend                   | Receives back ownership of buffers sent on `decryptOut`. |
| output        | bufferReturnOut    | Fw.BufferSend                   | Returns incoming iv/data buffers for deallocation. |
...
| sync input    | saDecryptIn        | [SdlsCfg.SaRouterPortCount] Fw.BufferSend | Receives decrypted data from downstream decryptors. |
| output        | saDecryptReturnOut | [SdlsCfg.SaRouterPortCount] Fw.BufferSend | Returns ownership of decrypted data buffers to downstream decryptors. |
| sync input    | saBufferReturnIn   | [SdlsCfg.SaRouterPortCount] Fw.BufferSend | Receives back iv/data buffers from downstream decryptors for deallocation. |
...
| SVC-Ccsds-SDLS-SA-ROUTER-002 | The SdlsSaRouter shall map the incoming SA index to a downstream port index using a compile-time SA-to-port map: an FPP array of {U16 SA, FwIndexType port index} pairs defined in configuration. | Unit Test |
