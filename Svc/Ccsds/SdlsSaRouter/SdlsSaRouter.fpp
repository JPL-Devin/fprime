...
module Ccsds {
    ...
    passive component SdlsSaRouter {
        ...
        sync input port saDecryptIn: [SdlsCfg.SaRouterPortCount] Fw.BufferSend

        @ Ports for returning ownership of decrypted data buffers to downstream decryptors
        output port saDecryptReturnOut: [SdlsCfg.SaRouterPortCount] Fw.BufferSend

        @ Ports for receiving back iv/data buffers from downstream decryptors for deallocation
        sync input port saBufferReturnIn: [SdlsCfg.SaRouterPortCount] Fw.BufferSend
    }
}
}
