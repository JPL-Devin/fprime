...
These checks build the external project against fprime `devel`, not against its pinned `lib/fprime` submodule. The pin is kept current by per-repository fprime submodule bump automation.
