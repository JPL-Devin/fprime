...
function(fpp_ut_setup_autocode MODULE_NAME AC_INPUT_FILES)
    ...
    get_target_property(FPRIME_SOURCE_LOCS "${FPRIME_PROJECT_INTERFACE_TARGET}" FPRIME_SOURCE_LOCATIONS)
    get_target_property(FPRIME_BINARY_LOCS "${FPRIME_PROJECT_INTERFACE_TARGET}" FPRIME_BINARY_LOCATIONS)
    if (NOT FPRIME_SOURCE_LOCS)
        set(FPRIME_SOURCE_LOCS "")
    endif()
    if (NOT FPRIME_BINARY_LOCS)
        set(FPRIME_BINARY_LOCS "")
    endif()
    set(FPRIME_BUILD_LOCS ${FPRIME_SOURCE_LOCS} ${FPRIME_BINARY_LOCS})
    string(REGEX REPLACE ";" ","  FPRIME_INCLUDE_DIRS_COMMA_SEP "${FPRIME_BUILD_LOCS}")
    ...
    endforeach()
    set(CLI_ARGS ${FPP_TO_CPP} "-u" "-d" "${CMAKE_CURRENT_BINARY_DIR}" ${IMPORTS} ${AC_INPUT_FILES} "-p" "${FPRIME_INCLUDE_DIRS_COMMA_SEP}")
