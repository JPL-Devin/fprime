...
The `Svc.ComDataWithContext` port type is used to transmit comms data between components of the F´ system. This port type is composed of a `Fw::Buffer` object and a `ComCfg::FrameContext` object. The `Fw::Buffer` object is used to transmit the data, while the `ComCfg::FrameContext` object is used to provide context for the data being transmitted, such as header information once the header has been consumed. The context's `saIndex` field carries the SDLS security association index (set by SDLS deframers and read by SDLS framers); the default `ComCfg.SaIndexUnset` (0xFFFF) is reserved to mean "unset". The context's `firstHeaderPointer` field carries the CCSDS TM First Header Pointer (set by `Svc.ComAggregator` when packet spanning is enabled and read by `Svc.Ccsds.TmFramer`); the default of 0 indicates a packet header at the start of the frame data field. The context's `tcSegmentHeaderPresent` and `tcSegmentHeader` fields carry the raw CCSDS TC Segment Header octet ([CCSDS 232.0-B-4](https://public.ccsds.org/Pubs/232x0b4e1c1.pdf) 4.1.3.2.2: bits 0-1 Sequence Flags, bits 2-7 MAP ID) when a TC deframer operating in Segment Header mode has stripped it; both default to `false`/`0` and are untouched when that mode is off. Adding these two fields grew `ComCfg::FrameContext::SERIALIZED_SIZE` by 2 octets; in-tree components only copy or pass the context by reference, but a project that serializes it into a buffer sized by a literal must account for the growth, and a project overriding `ComCfg.fpp` must add the two fields.
