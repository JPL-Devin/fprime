...
This file collects software design documents (SDDs) for F Prime components and
modules whose design warrants more detail than fits in source comments.
...
The implementation satisfies the following requirements, derived from the
`fprime-code-review` policy and the `Os::QueueInterface` contract:

- **R-MEM-1**: All memory required by the queue is allocated exactly once,
  during `create`, through the registered `Fw::MemAllocator`. No allocation
  occurs during `send`, `receive`, `getMessagesAvailable`, or
  `getMessageHighWaterMark`.
- **R-LOOP-1**: All non-blocking control paths terminate in time bounded by the
  configured queue depth multiplied by `MAX_RETRY_PASSES`. The high-water-mark
  CAS loop is bounded by `HIGH_MARK_CAS_BOUND`.
- **R-ISR-1**: The non-blocking variants of `send` and `receive` are safe to
  call from interrupt context. They use only lock-free atomic operations and
  bounded `memcpy`. They do not invoke any OS service.
- **R-ORD-1**: Consumers receive the slot with the highest priority. When
  multiple slots share the same priority, the slot with the smallest sequence
  number — i.e. the earliest publication — is selected. This matches the
  ordering contract of `Os::Generic::PriorityQueue`.
- **R-LOSS-1**: A successful `send` followed by any number of `receive` calls
  on cooperating threads delivers each message exactly once. No message is
  dropped, duplicated, or corrupted under concurrent producers and consumers.
- **R-API-1**: The public surface conforms to `Os::QueueInterface` and is
  selectable as the default `Os::Queue` implementation through
  `register_fprime_implementation` in CMake.

The implementation is **not** intended to be a general lock-free O(log n)
priority queue. Linear scans are accepted because flight queues are small (tens
of slots) and because the predictable, depth-bounded scan time is exactly the
property flight software wants.
...
compare-and-swap (CAS) operations; there is no shared writer lock or condition
variable anywhere in the data path.
...
The slot's non-atomic fields (`m_priority`, `m_size`, `m_sequence`) are written
by the producer while the slot is in `WRITING` and become visible to consumers
through the `release` store that publishes `READY`. Consumers acquire those
fields through a matching `acquire` load of `m_stateTag`.
...
immediately after reading the non-atomic priority/sequence fields catches the
...
```
prev = m_highMark.load(relaxed)
for pass = 0 .. HIGH_MARK_CAS_BOUND - 1:
    if next_count <= prev:
        break
    if CAS(m_highMark, prev -> next_count):
        break
```

In the worst case the loop terminates after `HIGH_MARK_CAS_BOUND` iterations,
...
- The transition `WRITING -> READY` uses `release` so that the non-atomic
  payload, priority, sequence, and size become visible to any consumer that
...
LeakSanitizer enabled, and pass under those sanitizers.
...
- The high-water mark is observability-only and may lag the true maximum by
  one under extreme contention, after `HIGH_MARK_CAS_BOUND` retries.
