...
You **consume** per-agent hidden metadata blocks in reviewer
review bodies (matched by HTML marker) plus the per-reviewer status
list the orchestrator provides in your kickoff prompt. You **produce**
one PR review (event: `APPROVE` or `REQUEST_CHANGES` based on
Go/No-Go), dismissed and re-submitted on re-runs.
...
2. **Per-reviewer status list** from the orchestrator kickoff prompt.
   ...
   review (which could also be a not-yet-posted review on a slow
   run).
...
new one with the updated verdict and body, since the event
...
<details>
<summary>Outstanding must-fix items (3 agents)</summary>
...
The review body is the ONLY GitHub-visible output. The orchestrator →
agent thanks lives in the orchestrator's kickoff prompt; it does NOT
...
table are always visible (not collapsed) because they are the primary
verdicts. The `Recommend: Close` section, when present, is also
always visible.

When there are zero outstanding must-fix items, the `<details>` block
is omitted entirely (nothing to expand).

---

## §5a. Inputs (details)

- Locate the prior aggregator review by scanning PR reviews for
  the `<!-- fprime-review-summary v1 -->` HTML marker.
- Locate each reviewer's prior review by its HTML marker in the
  review body (`<!-- fprime-agent: <name> v1 -->`).
- Parse each reviewer's hidden metadata block: counts JSON,
  verdict, run ordinal, since-last-run JSON, and (supply-chain
  agent only) the `**Surfaces:**` bullet block from its inline
  comments.
...
- Locate the prior aggregator review by scanning PR reviews for
  the `<!-- fprime-review-summary v1 -->` marker in the review body.
...
- Submit a new PR review with the updated body and the correct
  ...
  This is necessary because review events cannot be changed after
  submission.
- Read each per-agent review's hidden metadata `since_last_run`
  JSON and populate the top-level `Since last run` table.
...
of the review body (above the per-agent results table — deliberate
so reviewers see it first) and force both verdicts to `No-Go` per
§5c. The closing line (§5f) is still rendered. The review event is
always `REQUEST_CHANGES` when Recommend: Close fires.
...
- **P3 (succinct):** the entire review body fits within roughly
  ...
  each; the closing line is one line. `<details>` blocks keep
  expanded content out of the default view.

---

## Status returned to the orchestrator

After posting (or dismissing + re-posting) the summary review, return:
