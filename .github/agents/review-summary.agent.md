---
description: "Use to produce the consolidated F Prime multi-agent PR review summary. Consumes the per-agent summary reviews on a PR (from the security, supply-chain, C/C++ design, stale-documentation, design, architecture, and test-quality reviewers) and emits ONE top-level PR comment with a combined results table (one row per agent plus a CI safety row), a supply-chain surfaces drill-down table, merge readiness verdict, outstanding must-fix bullets, since-last-run delta, and (when triggered) a Recommend: Close section. Invoked by the orchestrator after the reviewers finish; not normally invoked directly."
