...
5. Report a single one-line status to the human operator:
   ...
   Followed by a link to the aggregator's summary review on the PR.
   **This is the only human-facing output.**
...
Aggregator. Please consume the per-agent hidden metadata and
inline comments on PR #<N> in <owner>/<repo> at head <sha> and
produce the consolidated PR review (APPROVE or REQUEST_CHANGES)
per `review-summary.agent.md` and the contract.
...
2. Inform the human operator in the orchestrator's final one-line
   status message. The summary review was not posted; the human
...
- The aggregator dismisses its prior review and submits a new one
  ...
  and updates the Since-last-run table.
