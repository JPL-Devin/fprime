...
//! queue. The non-atomic fields are written by the slot's owning producer while the slot is in
//! state `LOCKLESS_SLOT_WRITING` and read by the owning consumer while the slot is in state
//! `LOCKLESS_SLOT_READING`. Publication and acquisition are ordered through the atomic state-tag
//! using release/acquire semantics.
struct LocklessSlot {
    ...
    //! are equal. Written before the slot is published and read while the slot is `READY`.
    U32 m_sequence;
    //! Stored message size, less than or equal to the queue's configured message size.
    FwSizeType m_size;
    //! Stored message priority.
    FwQueuePriorityType m_priority;
